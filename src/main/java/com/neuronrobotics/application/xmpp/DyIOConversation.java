...
public class DyIOConversation implements IConversation, MessageListener, IChannelEventListener {
	 ...
	 *
	 * @see
	 * org.jivesoftware.smack.MessageListener#processMessage(org.jivesoftware.smack.
	 * Chat, org.jivesoftware.smack.packet.Message)
	 */
	 ...
	 *
	 * @see
	 * com.neuronrobotics.application.xmpp.IConversation#onMessage(java.lang.String,
	 * org.jivesoftware.smack.Chat, java.lang.String)
	 */
	@Override
	public String onMessage(String input, Chat chat, String from) {
		...
		if (packet[0].toLowerCase().contains("ping")) {
			...
			((DyIO) DeviceManager.getSpecificDevice(DyIO.class, null)).getChannel(port)
					.configAdvancedAsyncNotEqual(rate);
			((DyIO) DeviceManager.getSpecificDevice(DyIO.class, null)).getChannel(port)
					.addChannelEventListener(getListener(chat, from));
			return "async " + port + " " + rate;
		} else if (packet[0].toLowerCase().contains("removeasync")) {
			int port = Integer.parseInt(packet[1]);
			((DyIO) DeviceManager.getSpecificDevice(DyIO.class, null)).getChannel(port)
					.removeChannelEventListener(getListener(chat, from));
			return "async removed " + port + " ";
		} else if (packet[0].toLowerCase().contains("reset")) {
			for (int i = 0; i < 24; i++) {
				((DyIO) DeviceManager.getSpecificDevice(DyIO.class, null)).getChannel(i)
	...
	/**
	 * The listener interface for receiving chatAsync events. The class that is
	 * interested in processing a chatAsync event implements this interface, and the
	 * object created with that class is registered with a component using the
	 * component's addChatAsyncListener method. When the chatAsync event occurs,
