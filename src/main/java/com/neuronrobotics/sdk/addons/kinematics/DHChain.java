...
public class DHChain extends TimeKeeper {
	...
	// public DHChain(double [] upperLimits,double [] lowerLimits, boolean
	// debugViewer ) {
	...
	// getLinks().add(new DHLink( 13, Math.toRadians(180), 32,
	// Math.toRadians(-90)));//0->1
	// getLinks().add(new DHLink( 25, Math.toRadians(-90), 93,
	// Math.toRadians(180)));//1->2
	// getLinks().add(new DHLink( 11, Math.toRadians(90), 24,
	// Math.toRadians(90)));//2->3
	// getLinks().add(new DHLink( 128, Math.toRadians(-90), 0,
	// Math.toRadians(90)));//3->4
	//
	// getLinks().add(new DHLink( 0, Math.toRadians(0), 0,
	// Math.toRadians(-90)));//4->5
	// getLinks().add(new DHLink( 25, Math.toRadians(90), 0,
	// Math.toRadians(0)));//5->tool
	//
	...
	public double[] inverseKinematics(TransformNR target, double[] jointSpaceVector) throws Exception {
		...
		// Log.info( "Inverse Kinematics took
	...
	public Matrix forwardKinematicsMatrix(double[] jointSpaceVector, ArrayList<TransformNR> chainToLoad) {
		...
		if (jointSpaceVector.length != getLinks().size())
			throw new IndexOutOfBoundsException("DH links do not match defined links. expected " + getLinks().size()
		...
		if (is == null) {
			...
			return new DhInverseSolver() {
				...
				public double[] inverseKinematics(TransformNR target, double[] jointSpaceVector, DHChain chain) {
					...
					// this is an ad-hock kinematic model for d-h parameters and only works for
					...
					TransformNR overGripper = new TransformNR(xSet, ySet, zSet, target.getRotation());
					...
					/*
					 * println ( "TO: "+target); println ( "Trangular TO: "+overGripper); println (
					 * "lengthXYPlaneVect: "+lengthXYPlaneVect); println(
					 * "angleXYPlaneVect: "+Math.toDegrees(angleXYPlaneVect)); println(
					 * "angleRectangleAdjustedXY: "+Math.toDegrees(angleRectangleAdjustedXY));
					 * println( "lengthRectangleAdjustedXY: "+lengthRectangleAdjustedXY); println(
					 * "r: "+r); println( "d: "+d);
					 *
					 * println( "x Correction: "+xSet); println( "y Correction: "+ySet);
					 *
					 * println( "Orientation: "+Math.toDegrees(orientation)); println( "z: "+zSet);
					 */
					...
					/*
					 * println( "vect: "+vect); println( "A: "+Math.toDegrees(A)); println(
					 * "elevation: "+Math.toDegrees(elevation)); println(
					 * "l1 from x/y plane: "+Math.toDegrees(A+elevation)); println(
					 * "l2 from l1: "+Math.toDegrees(C));
					 */
