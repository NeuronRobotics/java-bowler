...
public class DyIOChannel implements IDyIOChannel {
	 ...
	 *
	 * @param dyio
	 *            The DyIO that the channel belongs on
	 * @param channel
	 *            The Channel on the DyIO that this object corresponds to.
	 * @param mode
	 *            The Type of channel.
	 * @param isEditable
	 *            Lock the channel parameters after creation.
	 */
	...
	/**
	 * This method allows the user to update all of the content information of this
	...
	/**
	 * Gets the mode of the channel. If resync is true, then the channel will do a
	 * live query to the device for its current mode and cache it to the channel for
	 * future use.
	 *
	 ...
	 *
	 * @param all
	 *            - should all channels be refreshed.
	 */
	...
	/**
	 * This method gets a collection of all of the possible channel modes for this
	 * channel.
	 */
	...
	public Collection<DyIOChannelMode> getAvailableModes() {
		...
		if (getDevice().isServoPowerSafeMode()) {
			if (number < (getDevice().getDyIOChannelCount() / 2)
					&& device.getBankAState() != DyIOPowerState.REGULATED) {
				myModes.add(DyIOChannelMode.SERVO_OUT);
			}

			if (number >= (getDevice().getDyIOChannelCount() / 2)
					&& device.getBankBState() != DyIOPowerState.REGULATED) {
				myModes.add(DyIOChannelMode.SERVO_OUT);
			}
		} else {
			myModes.add(DyIOChannelMode.SERVO_OUT);
	...
	/**
	 * This method gets the value represented by the date portion of a
	 * DyIOChannelEvent.
	 *
	 * @param e
	 *            the event to parse
	 ...
	 *
	 * @param e
	 *            the event to pass to all listeners
	 */
	...
	public int getValue() {
		...
		if (getDevice().isLegacyParser()) {
			...
			// For the new API the channel values should come in through the asynchronous
			// path.
	 ...
	 *
	 * @see
	 * com.neuronrobotics.sdk.dyio.IDyIOChannel#setMode(com.neuronrobotics.sdk.dyio.
	 * DyIOChannelMode, boolean)
	 */
	...
	public boolean setMode(DyIOChannelMode mode, boolean async) {
		...
		if (!canBeMode(mode)) {
			if (mode == DyIOChannelMode.SERVO_OUT)
				new RuntimeException("\nChannel: " + getChannelNumber() + " can not be mode '" + mode
						+ "' in current configuration. \nCheck the power switch settings and available modes.")
						.printStackTrace();
			else
				new RuntimeException("\nChannel: " + getChannelNumber() + " can not be mode '" + mode + "'.")
						.printStackTrace();
		...
		for (int i = 0; i < MAXATTEMPTS; i++) {
			try {
				...
				if (getDevice().isLegacyParser()) {
					...
					// Log.enableInfoPrint();
					Object[] args = getDevice().send("bcs.io.setmode.*;0.3;;", BowlerMethod.POST, "schm",
							new Object[]{getChannelNumber(), mode.getValue(), async ? 1 : 0});
					ByteList currentModes = (ByteList) args[0];
					// com.neuronrobotics.sdk.common.Log.error("Setting # "+getChannelNumber()+" to
					// "+mode);
	...
	public boolean flush() {
		...
		if (getDevice().isLegacyParser()) {
			...
			getDevice().send("bcs.io.*;0.3;;", BowlerMethod.POST, "schv",
	...
	/**
	 * THis method allows the user to set a chached value to be sent to the device
	 * when a flush is called later on. THis method will not send this value to the
	 * device, a flush must be called later to send it.
	 *
	 * @param cachedValue
	 *            the value to store as the cached value
	 */
	...
	/**
	 * THis sets up the abstract peripheral object that is using this channel
	 * object.
	 *
	 * @param dap
	 *            the new dap
	...
	/**
	 * This method provides access to the abstract perpheral that will be using this
	 * channel object.
	 *
	 * @return the dap
	 */
	...
	/**
	 * Sets the time to store for use by the flush. THis will only be used when
	 * flush is called from the channel If flush is called from the DyIO, time
	 * stored here will be ignored
	 *
	 * @param cachedTime
	 *            the new cached time
	 */
	 ...
	 *
	 * @see
	 * com.neuronrobotics.sdk.dyio.IDyIOChannel#setValue(com.neuronrobotics.sdk.
	 * common.ISendable)
	 */
	...
	public boolean setValue(ByteList data) {
		...
		if (getDevice().isLegacyParser()) {
			...
			getDevice().send("bcs.io.*;0.3;;", BowlerMethod.POST, "strm", new Object[]{number, data});
		}
