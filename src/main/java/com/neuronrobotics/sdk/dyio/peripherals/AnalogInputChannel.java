...
public class AnalogInputChannel extends DyIOAbstractPeripheral implements IChannelEventListener {
	...
	/**
	 * Constructor. Creates an analog input channel that is syncronous only by
	 * default.
	 *
	 * @param channel
	...
	/**
	 * Constructor. Creates an analog input channel with the given ability for
	 * asyncronous communications.
	 *
	 * @param channel
	 *            - the channel object requested from the DyIO
	 * @param async
	 *            - boolean to determine if this is an async analog channel
	 */
	public AnalogInputChannel(DyIOChannel channel, boolean async) {
		...
		if (!setMode(async)) {
			throw new DyIOPeripheralException(
					"Could not set channel " + getChannel() + " to " + DyIOChannelMode.ANALOG_IN + " mode.");
		}
	}
	...
	public void onChannelEvent(DyIOChannelEvent e) {
		fireValueChanged(e.getUnsignedValue());
	}
