...
public abstract class DyIOAbstractPeripheral implements IDyIOChannel {
	 ...
	 *
	 * @return the configuration from device
	 */
	private int getConfigurationFromDevice() {
		if (getChannel().getDevice().isLegacyParser()) {
			...
			Object[] args = getChannel().getDevice().send("bcs.io.*;0.3;;", BowlerMethod.CRITICAL, "cchn",
					new Object[]{255, false, new Integer[]{}});
	...
	public boolean setMode(DyIOChannelMode mode, boolean async) {
		if (mode != getClassMode())
			throw new RuntimeException(
					"The mode being set does not match the defined channel mode: " + mode + " is not" + getClassMode());
		return channel.setMode(getClassMode(), async);
	}
	...
	/**
	 * This method sets the value of the output of the giver peripheral, and also
	 * stores this value as the "default" value in non volatile memory to use at
	 * startup of the peripheral.
	 *
	 * @param pos
		...
		try {
			...
			switch (mode) {
				...
				case PWM_OUT :
					...
					if (getChannel().getDevice().isLegacyParser()) {
						getChannel().send(
								new SetChannelValueCommand(getChannel().getChannelNumber(), pos, getMode(), true));
					} else {

						getChannel().getDevice().send("bcs.io.*;0.3;;", BowlerMethod.CRITICAL, "cchn",
								new Object[]{getChannel().getChannelNumber(), true, new Integer[]{pos}});
	...
	/**
	 * This method is to check if this peripheral is capable to be set as up as
	 * asynchronous.
	 *
