...
public class ServoChannel extends DyIOAbstractPeripheral implements IChannelEventListener {
	...
	public ServoChannel(DyIOChannel channel) {
		...
		if (!setMode()) {
			throw new DyIOPeripheralException(
					"Could not set channel " + channel + " to " + DyIOChannelMode.SERVO_OUT + " mode");
		}
		channel.setDap(this);
	 ...
	 *            - the end position
	 * @param time
	 *            - the number of seconds for the transition to take place
	 * @return if the action was successful
	 */
	...
	/**
	 * This method allows you to override the servo voltage lock-out it is enabled
	 * by default.
	 */
