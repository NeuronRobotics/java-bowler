...
/**
 * The DyIO class is an encapsulation of all of the functionality of the DyIO
 * module into one object. This object has one connection to one DyIO module and
 * wraps all of the commands in an accessible API.
 *
...
public class DyIO extends BowlerAbstractDevice implements IPidControlNamespace, IConnectionEventListener, IFlushable {
	...
	/**
	 * Default Constructor. Builds a generic DyIO that has the default broadcast
	 * address and no default connection.
	 */
	public DyIO() {
	 ...
	 *
	 * @param channel
	 *            integer representing the index of the channel - a channel number
	 * @return the channel
	 */
	 ...
	 *
	 * @see
	 * com.neuronrobotics.sdk.common.BowlerAbstractDevice#send(java.lang.String,
	 * com.neuronrobotics.sdk.common.BowlerMethod, java.lang.String,
	 * java.lang.Object[])
	 */
	...
	/**
	 * This method sets the I/O mode of a specific channel. the DyIO will be sent a
	 * packet to update its mode in firmware If the mode is the same as the current
	 * mode, nothing happens If the mode is different from the previous mode, a mode
	 * change listeners will be fired Asynchronus state will be unaffected
	 *
	 * @param channel
	 *            integer representing the index of the channel
	 * @param mode
	 *            the DyIOChannelMode that this channel should be set to
	 ...
	 *            the DyIOChannelMode that this channel should be set to
	 * @param async
	 *            Forces the mode into or out of async mode
	 * @return true for success
	 */
	 ...
	 *            integer representing the index of the channel
	 * @param value
	 *            Unit-less value to set to the DyIO's channel
	 * @return true for success
	 */
	...
	/**
	 * This method sets the value of a channel with a pre-packaged ByteList of the
	 * data to send. Since the bytelist is pre-packaged, the data will be sent to
	 * the DyIO as it is packet into the ByteList with NO VERIFICATION.
	 *
	 * @param channel
	 *            integer representing the index of the channel
	 * @param value
	 *            ByteList of the data to send to the DyIO
	 * @return true for success
	 */
	...
	/**
	 * Gets the DyIO's internally stored info string. This is a 16 byte string that
	 * can be stored on the DYIO and used as a human-readable, user setable,
	 * identifier.
	 *
	...
	/**
	 * Sets the DyIO's internally stored info string. This is a 16 byte string that
	 * can be stored on the DYIO and used as a human-readable, user setable,
	 * identifier.
	 *
	 * @param info
	 *            The String identifier to be stored by the DyIO.
	 */
	...
	/**
	 * This method gets the 3 byte firmware revision code. THis code is used to
	 * determine compatibility between the version of firmware on the DyIO and the
	 * version of the NRDK being used to communicate with it.
	 *
	...
	/**
	 * This method turns the firmware version number into a formatted string for
	 * clear display
	 *
	...
	/**
	 * This method creates a new collection and populates it with the DyIO channel
	 * objects.
	 *
	...
	public ArrayList<DyIOChannel> getChannels() {
		...
		for (DyIOChannel chan : getInternalChannels()) {
			// com.neuronrobotics.sdk.common.Log.error(this.getClass()+" Adding channel:
			// "+chan.getChannelNumber()+" as mode: "+chan.getMode());
			c.add(chan);
		}
		return c;
	}
	...
	/**
	 * This method synchronizes the DyIO channel mode from the DyIO module with this
	 * DyIO channel object This will actively query the DyIO for this information.
	 *
	 * @param channel
	 *            integer representing the index of the channel
	 */
	...
	/**
	 * This static method can be called before connection a DyIO object to disable
	 * the firmware verification step This can be used to allow older versions of
	 * the DyIO firmware to be used with newer NRDK versions.
	 */
	...
	/**
	 * This static method can be called before connection a DyIO object to enable
	 * the firmware verification step.
	 */
	...
	/**
	 * Sync the state cache with the live device. This method will query the device
	 * for its firmware revision and its info string. The default opperation will be
	 * to throw a DyIOFirmwareOutOfDateException is the firmware version does not
	 * match the NRDK build version. This can be overridden if DyIO.disableFWCheck()
	 * is called BEFORE connection.
	 *
	 * @throws DyIOFirmwareOutOfDateException
	 *             the dy io firmware out of date exception
	 */
			...
			try {
				...
				if (getInternalChannels().size() == 0) {
					...
					try {
						...
						throw new DyIOCommunicationException(
								"DyIO failed to report during initialization. Could not determine DyIO configuration: "
										+ e2.getMessage());
					}
				} else {
					setMuteResyncOnModeChange(false);
	...
	/**
	 * This method re-synchronizes the entire state of the DyIO object with the DyIO
	 * module. The Firmware version is checked The info string is checked All
	 * channel modes are updated
	 *
	...
	/**
	 * Add an IDyIOEventListener that will be contacted with an IDyIOEvent on each
	 * incoming data event.
	 *
	 * DyIO event listeners are used to get information from all DyIO events. This
	 * is how to access the Power events: DyIO power switch change events DyIO
	 * external power voltage change events
	 *
	 * @param l
	 *            the l
	 */
	...
	protected void validateChannel(int channel) {
		...
		if (channel < 0 || channel > getInternalChannels().size()) {
			throw new IndexOutOfBoundsException(
					"DyIO channels must be between 0 and " + getInternalChannels().size() + " requested:" + channel);
		}
	}

	/**
	 * This method returns the current state of the DyIO objects cache/flush system
	 * for the DyIO channel values.
	 *
	 ...
	 * packets to the DyIO. The channels will then need to either be flushed
	 * individually, or as a group. When the entire group is flushed, all channel
	 * values are set with a single packet allowing for co-ordinated motion. All
	 * channels will read the current state and set it as the cache value on the
	 * event of enabling the cache/flush mode
	 *
	 * @param mode
	 *            true to enable cache/flush mode, false to disable.
	 */
	...
	/**
	 * This method will flush the DyIO cache for all channels. All channel values as
	 * stored by setting the value from code, or the value stored at the time that
	 * the cache/flush mode was enabled. THis method will flush all 24 channel
	 * values in one packet allowing for co-ordinated motion.
	 *
	 * @param seconds
	 *            the seconds
	 */
	public void flushCache(double seconds) {
		...
		for (DyIOChannel d : getInternalChannels()) {
			...
			// com.neuronrobotics.sdk.common.Log.error("Flushing chan "+d+" to
			// "+d.getCachedValue());
		}
		if (isLegacyParser()) {
			for (int j = 0; j < 5; j++) {
			...
			// d.flush();
			// }
			send("bcs.io.*;0.3;;", BowlerMethod.POST, "sacv",
	...
	//
	/**
	 * This method returns the bank switch state of bank A (0-11) This state is
	 * updated asynchronously by the DyIOEventListener.
	 *
	...
	/**
	 * This method returns the bank switch state of bank B (12-23) This state is
	 * updated asynchronously by the DyIOEventListener.
	 *
	 * @return the current state
	 */
	...
	/**
	 * THis method will return the current voltage on the battery connected to the
	 * DyIO external power connector.
	 *
	 * @param refresh
	 *            true if you want to query the device, false to just get the cached
	 *            value from the last async.
	...
	/**
	 * This method sends a packet to the DyIO module to set up the linking between a
	 * DyIO input channel and a DyIO output channel to a PID controller Inputs are
	 * read as the input to the PID calculation Outputs are set as a result of the
	 * PID calculation.
	 *
	 * @param config
	...
	/**
	 * This method gets the current state of the DyIO channel configuration of a
	 * given PID group.
	 *
	 * @param group
	 *            the index of the PID group to get information about
	 ...
	 */
	@Override
	public boolean SetPIDInterpolatedVelocity(int group, int unitsPerSecond, double seconds)
	...
	/*
	 * Advanced Async Configuration. THese methods configure what value changes
	 * trigger an asynchronous event as well as how often those event states are
	 * checked
	 */
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the values are not equal. This sets the sample
	 * time to 100 ms
	 *
	 * @param pin
	 *            the DyIO channel to configure
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the values are outside a deadband This sets the
	 * sample time to 100 ms.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @param deadbandSize
	 *            the size in sensor units of the deadband
	 * @return true if success
	 */
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the value crosses a threshhold This sets the
	 * sample time to 100 ms.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @param threshholdValue
	 *            a value setpoint that triggers an even when it is crossed
	 * @param edgeType
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the value is sampled on a real-time This sets the
	 * sample time to 100 ms clock and sent as async regardless of value change.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @return true if success
	 ...
	 *            the DyIO channel to configure
	 * @param time
	 *            the sample time in MiliSeconds
	 * @return true if success
	 */
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the values are outside a deadband.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @param time
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the value crosses a threshhold.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @param time
	 ...
	 * @return true if success
	 */
	public boolean configAdvancedAsyncTreshhold(int pin, int time, int threshholdValue,
	...
	/**
	 * This method configures the advanced async mode for a given DyIO channel to
	 * trigger on any event where the value is sampled on a real-time clock and sent
	 * as async regardless of value change.
	 *
	 * @param pin
	 *            the DyIO channel to configure
	 * @param time
	...
	/**
	 * This method enables the heart beat. It tells the device how often to expect
	 * synchronous packet, and if communication fails to send in the given time,
	 * then the device should go into its "safe state". This will also set up a
	 * thread to ping the device periodically if it has been too long since the last
	 * user generated synchronous packet.
	 *
	 * @param msHeartBeatTime
	 *            the ms heart beat time
	 */
	...
	/**
	 * This method stops the heart beat and tells the device to stop expecting a
	 * heart beat. This will DISABLE the safe mode detect.
	 */
	...
	// /**
	// * This method allows you to disable the brown out detect for the servo
	// subsystem. If true is passed
	 ...
	 *
	 * @param enable
	 *            true to enable the borwnout, false to disable
	 * @return True is success
	 */
	...
	/**
	 * This will enable a state where the DyIO will surpress DyIO mode update
	 * events.
	 *
	 * @param muteResyncOnModeChange
	 *            true to enable the muted mode
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.neuronrobotics.sdk.common.IConnectionEventListener#onDisconnect(com.
	 * neuronrobotics.sdk.common.BowlerAbstractConnection)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.neuronrobotics.sdk.common.IConnectionEventListener#onConnect(com.
	 * neuronrobotics.sdk.common.BowlerAbstractConnection)
	 */
	@Override
	...
	/**
	 * Sets the flag to represent if the DyIO is currently re-syncing itself with
	 * the device.
	 *
	 * @param resyncing
	 *            the new resyncing
	...
	public String toString() {
		...
		for (int i = 0; i < getInternalChannels().size() / 2; i++) {
			...
			s += String.format(chFmt, oppisite, getInternalChannels().get(oppisite).getPreviousValue(),
					getInternalChannels().get(oppisite).getMode().toSlug(),

					i, getInternalChannels().get(i).getPreviousValue(),
	 ...
	 * ConfigurePDVelovityController(com.neuronrobotics.sdk.pid.
	 ...
	 *
	 * @see com.neuronrobotics.sdk.namespace.bcs.pid.IPidControlNamespace#
	 * getPDVelocityConfiguration(int)
	 */
	@Override
