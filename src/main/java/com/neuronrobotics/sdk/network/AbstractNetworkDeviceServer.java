...
public class AbstractNetworkDeviceServer extends BowlerAbstractServer {
	 ...
	 */
	public AbstractNetworkDeviceServer(GenericDevice device, boolean useAsync,
			BowlerAbstractConnection serverConnection) {
		super(device.getAddress());
		gen = device;
		namespaces = gen.getNamespaces();
		for (int i = 0; i < namespaces.size(); i++) {
			BowlerAbstractDeviceServerNamespace ns = new BowlerDeviceReServerNamespace(device.getConnection(), this,
					useAsync, i, namespaces.get(i), gen);
