...
public class NRBoot {
	...
	public boolean load(Core core) {
		...
		if (id == null) {
			...
			// com.neuronrobotics.sdk.common.Log.error("Bootloader
			// ID:"+core.getType().getReadableName());
		} else {
	...
	private void send(IntelHexParser parse, int core) {
		...
		while (line != null) {
			...
			if (!boot.write(core, line)) {
				// com.neuronrobotics.sdk.common.Log.error("Failed to write, is the device in
				// bootloader mode?");
				return;
			}
