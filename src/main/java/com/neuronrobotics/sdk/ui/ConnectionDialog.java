...
public class ConnectionDialog extends JDialog {
		...
		connectBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					JOptionPane.showMessageDialog(null, "Error connecting with the given connection.",
	...
	/**
	 * Displays the dialog and blocks until the user has chosen 'Set', 'Cancel', or
	 * 'No Connection' Returns true if the user set a connection, returns false
	 * otherwise.
	 *
	...
	public BowlerAbstractConnection getConnection() {
		...
		BowlerAbstractConnection c = ((AbstractConnectionPanel) connectionPanels.getSelectedComponent())
				.getConnection();
		if (c == null) {
			JOptionPane.showMessageDialog(null, "Unable to create connection.", "Invalid Connection",
	...
	public static boolean getBowlerDevice(BowlerAbstractDevice dev, AbstractConnectionPanel panel) {
		...
		while (connection == null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, m, "DyIO Not Responding" + e1.getMessage(),
						JOptionPane.ERROR_MESSAGE);
				continue;
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(null, e.getMessage(), "DyIO Connection problem ",
						JOptionPane.ERROR_MESSAGE);
				return false;
			}
	...
	/**
	 * Displays a serial connection dialog to the user and returns the connection or
	 * null.
	 *
	 * @return the connection if one is selected, null if canceled or no connection
	 *         is selected.
	 */
