...
/**
 * Connections create a bridge between a device and the SDK. Each connection is
 * encapsulated to allow maximum reuse and system changes without the need to
 * restart / reconfigure.
 *
 */
public abstract class BowlerAbstractConnection {
	 ...
	 *
	 * @param up
	 *            the new threaded upstream packets
	 */
	...
	/**
	 * Sends any "universal" data to the connection and returns either the
	 * syncronous response or null in the event that the connection has determined a
	 * timeout. Before sending, use clearLastSyncronousResponse() and use
	 * getLastSyncronousResponse() to get the last response since clearing.
	 *
	 * @param sendable
	 *            the sendable
	...
	public synchronized BowlerDatagram sendSynchronusly(BowlerDatagram sendable, boolean switchParser) {
		...
		} while (((System.currentTimeMillis() - startOfReciveTime) < getSleepTime())
				&& (getLastSyncronousResponse() == null));
		long rcvTime = (System.currentTimeMillis() - startOfReciveTime);

		if (rcvTime > (getSleepTime() * getPercentagePrint() / 100)) {
			Log.warning("Receive took: " + rcvTime + " ms. This is greater then " + getPercentagePrint()
		...
		if (getLastSyncronousResponse() == null) {
			Log.error("No response from device, no response in " + (System.currentTimeMillis() - startOfReciveTime)
					+ " ms");
	...
	public synchronized void setConnected(boolean c) {
		...
		if (connected) {
			...
			/*
			 * if( BowlerDatagram.isUseBowlerV4()){ //If the ping fails to get a response,
			 * try the older bowler format Log.warning("Switching to legacy parser");
			 * BowlerDatagram.setUseBowlerV4(false); }else{
			 * Log.warning("Switching to v4 parser"); BowlerDatagram.setUseBowlerV4(true); }
			 * if(!ping(new MACAddress())){ //neither packet format is working, bail out
			 * setConnected(false); } }
			 */
			fireConnectEvent();
			Runtime.getRuntime().addShutdownHook(new Thread() {
				...
				public void run() {
					if (isConnected()) {
						// com.neuronrobotics.sdk.common.Log.error("WARNING: Bowler devices should be
	...
	/**
	 * Whenever a connection has received a full datagram from its "physical"
	 * connection, it should then call onDataReceived with the datagram. This will
	 * set it to the last received data if it is synchronous and will add it to the
	 * appropriate queues to be picked up and send to the listeners.
	 *
	 * @param data
	 *            the data
	 */
	 ...
	 *
	 * @param l
	 *            the new synchronous datagram listener
	 */
	 ...
	 * @return the command
	 */
	public static BowlerAbstractCommand getCommand(String namespace, BowlerMethod method, String rpcString,
			Object[] arguments, RpcEncapsulation rpc) {
	 ...
	 */
	public Object[] send(MACAddress addr, String namespace, BowlerMethod method, String rpcString, Object[] arguments,
			int retry) throws DeviceConnectionException {
		...
		throw new DeviceConnectionException(
				"Device does not contain command NS=" + namespace + " Method=" + method + " RPC=" + rpcString + "'");
	}
			...
			while (!done) {
				...
				try {
					...
					if (tmpNs.length() == namespacePacket.getData().size()) {
						...
						// com.neuronrobotics.sdk.common.Log.error("Ns = "+tmpNs+" len =
						// "+tmpNs.length()+" data = "+b.getData().size());
	...
	public ArrayList<RpcEncapsulation> getRpcList(String namespace, MACAddress addr) {
		...
		try {
			...
			for (int i = 0; i < numRpcs; i++) {
				...
				try {
					tmpRpc = new RpcEncapsulation(namespaceIndex, namespace, rpcStr, downstreamMethod, downArgs,
							upstreamMethod, upArgs);
	 ...
	 */
	public BowlerDatagram send(BowlerAbstractCommand command, MACAddress addr, int retry)
	 ...
	 *             the invalid response exception
	 */
	public BowlerDatagram send(BowlerAbstractCommand command, MACAddress addr, int retry, boolean switchParser)
			throws NoConnectionAvailableException, InvalidResponseException {
		for (int i = 0; i < retry; i++) {
			...
			try {
				...
				Log.error("Sending Synchronus packet and there was a failure, will retry " + (retry - i - 1)
						+ " more times");
	 ...
	 *             the invalid response exception
	 */
	public BowlerDatagram send(BowlerAbstractCommand command, MACAddress addr)
			throws NoConnectionAvailableException, InvalidResponseException {
	 ...
	 *             the invalid response exception
	 */
	public BowlerDatagram send(BowlerAbstractCommand command, MACAddress addr, boolean switchParser)
			throws NoConnectionAvailableException, InvalidResponseException {
		// if(!isConnected()) {
		// if(!connect())
	...
	/**
	 * Implementation of the Bowler ping ("_png") command Sends a ping to the device
	 ...
	 *
	 * @param percentagePrint
	 *            the new percentage print
	 */
				...
				if (queueBuffer.isEmpty()) {
					...
					try {
						...
						if (b != null) {
							...
							if ((System.currentTimeMillis() - getLastWrite()) > (getSleepTime()
									* (getPercentagePrint() / 100.0)) && b.isSyncronous()) {
								Log.error("Packet recive took more then " + getPercentagePrint() + "%. "
										+ "\nPacket Update\t" + (packetUpdate - start) + "" + "\nPulled Packet\t"
										+ (pulledPacket - packetUpdate) + "" + "\nPushed Packet\t"
										+ (pushedPacket - pulledPacket));
							}
						}
					} catch (Exception e) {
						e.printStackTrace();
	 ...
	 *
	 * @param useThreadedStack
	 *            the new use threaded stack
	 */
