...
public class BowlerDatagram implements ISendable, IthreadedTimoutListener {
	...
	private static byte genCRC(int index, int len, ByteList data) {
		...
		try {
			...
			throw new IndexOutOfBoundsException(
					"Attempting from: " + index + " to: " + len + " in: " + data.size() + " data: " + data);
		}
	 ...
	 * Data must be at least the size of a standard Bowler packet header (11 bytes
	 * long)
	 *
	 * @param data
	 *            the data
	...
	public void parse(ByteList raw) {
		...
		if (raw.getByte(0) != REVISION) {
			throw new MalformattedDatagram(
					"Datagram is revision " + raw.getByte(0) + ". Must be of revision " + REVISION);
		}
		...
		if ((dataLength != raw.getBytes(11).length - 1) && (dataLength != raw.getBytes(11).length)) {
			throw new MalformattedDatagram(
					"Datagram payload length is mismatched expected " + dataLength + " got " + raw.getBytes(11).length);
		}
	...
	public boolean isSyncronous() {
		...
		// Log.error("Device firmware out of date, should be using
		// BowlerMethod.ASYNCHRONOUS rather than transactionID != 0" + this);
		// }
	...
	private int getSessionID() {
		checkValidPacket();
		return getNamespaceResolutionID() >= 0
				? (int) getNamespaceResolutionID()
				: (int) getNamespaceResolutionID() + 256;
	}
	 ...
	 *
	 * @param namespaceResolutionID
	 *            the new namespace resolution id
	 */
	 ...
	 *
	 * @see
	 * com.neuronrobotics.sdk.common.IthreadedTimoutListener#onTimeout(java.lang.
	 * String)
	 */
	@Override
	public void onTimeout(String message) {
		if (!isFree()) {
			...
			if (timeout.getAmountOfTimeForTimerToRun() < timeoutTime) {
				...
				Log.error("Packet fucked up. Expected " + timeout.getAmountOfTimeForTimerToRun() + " ms, took "
						+ timeoutTime + " ms");
				timeout.initialize(BowlerDatagramFactory.getPacketTimeout(), this);
			}
		}
