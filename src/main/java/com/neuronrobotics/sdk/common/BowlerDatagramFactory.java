...
public class BowlerDatagramFactory {
	...
	public static BowlerDatagram build(MACAddress addr, BowlerAbstractCommand cmd) {
		...
		try {
			...
			Log.error("Timeout detected, duration = " + (System.currentTimeMillis() - start) + ", expected = "
					+ BowlerDatagramFactory.getPacketTimeout());
		}
	...
	private static BowlerDatagram build(ByteList buffer, BowlerDatagram staticMemory) {
		...
		while (fb != BowlerDatagram.REVISION) {
			// Log.error("First Byte Fail, Junk byte: "+String.format("%02x ",
			// buffer.pop()));
			failed++;
			try {
				if (buffer.size() == 0) {
		...
		while (check == false) {
			try {
				if ((buffer.get(0) != BowlerDatagram.REVISION) || (!BowlerDatagram.CheckCRC(buffer, false))) {
					if (buffer.get(0) != BowlerDatagram.REVISION)
						Log.error(
								"First Byte Fail (second attempt) Junk byte: " + String.format("%02x ", buffer.pop()));
					else
