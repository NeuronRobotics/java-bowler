...
public class ByteList implements ISendable, Iterable<Byte> {
	...
	/**
	 * Initial value constructor. Constructs a ByteList and populates it with the
	 * byte
	 *
	 * @param data
	 *            the initial data to load into the bytelist after construction
	 */
	...
	/**
	 * Adds each member of a given array to the ByteList. If the addition failed,
	 * then any added bytes are removed.
	 *
	 * @param data
	 *            the data
	 * @return if the addition was successful
	...
	/**
	 * Adds each value of a given array to the ByteList. If the addition failed,
	 * then any added bytes are removed.
	 *
	 * @param data
	 *            the data
	 ...
	 * This method should be avoided as it will be removed with 1.0 and add16 or
	 * add32 should be used instead.
	 *
	 * @param data
	 *            the data
	 * @param split
	...
	public synchronized byte[] getBytes(int start, int len) {
		...
		if (len < 0) {
			throw new RuntimeException("Requesting more bytes then in the list, size=" + sizeLocal + " start=" + start
					+ " len=" + len + " requested=" + requestedLen);
		}
	...
	/**
	 * Removes bytes from the start of the buffer upto and including the given
	 * index. If the index is greater than the size of the buffer, null is returned
	 * otherwise the byte at the given index is returned.
	 *
	 * @param index
	 *            the index
	 * @return the byte
	...
	/**
	 * Removes the first byte from the buffer and returns it. If the buffer is
	...
	/**
	 * toInt Takes a ByteList and turns it into the int that the stream represents.
	 * Assumes the 0th element is the most significant byte Assumes the entire
	 * stream is one number
	 *
	 * @param b
	 *            The byte array with the byte data in it
	 * @param Signed
	 *            If the stream should be treated as a signed integer
	 ...
	 *
	 * @param useStaticBuffer
	 *            the new use static buffer
	 */
