...
public interface IPidControlNamespace {
	/**
	 * This method calls a reset of the PID group. This will set the current value
	 * of the controllers input to the given value (if possible) and will set the
	 * setpoint of the PID group to the current value (stopping the output)
	 *
	 * @param group
	 *            the index of the PID group
	 * @param valueToSetCurrentTo
	 *            the target value that the controller should be set to.
	 ...
	 * PIDConfiguration ojbect prior to calling this method will be sent to the
	 * device.
	 *
	 * @param config
	 *            the configuration wrapper object
	 * @return true if success
	 */
	 ...
	 * target position which will take the specified number of seconds to make that
	 * transition
	 *
	 * @param group
	 *            the index of the PID group
	 * @param setpoint
	 *            the target position for the controller
	 * @param seconds
	 *            units in Seconds, the time it takes to make the transition from
	 *            current to target. Zero will tell the controller to go as fast as
	 *            possible.
	 ...
	 *
	 * @param setpoints
	 *            and array of setpoint values (must match the number of available
	 *            PID control groups)
	 * @param seconds
	 *            units in Seconds, the time it takes to make the transition from
	 *            current to target. Zero will tell the controllers to go as fast as
	 *            possible.
	 * @return true if no errors occur
	 */
	 ...
	 * many PID control groups are available on a device.
	 *
	 * @return and array of values representing the current state of the given
	 *         cntrollers input
	 */
	 ...
	 * Events include: PID reset, where the user is notified if the controllers
	 * input is reset from software PID limit, if the device generates a Home, Upper
	 * limit, or Lower limit event from a hardware event PID position, if the
	 * current position of the PID controllers sensor input changes.
	 *
	 * @param l
	 *            the l
	 */
	...
	/**
	 * This method will read all of the cached or current setpoints for all PID
	 * controllers and calls SetAllPIDSetPoint with its internal data.
	 *
	 * @param time
	 *            the time
	 */
	 ...
	 * value representable by the sensor and can fail if that value is out of range.
	 *
	 * @param group
	 *            the index of the PID group
	 * @param unitsPerSecond
	 *            a velocity in raw units per second
	 * @param seconds
	 *            the amount of time to run at this velocity
	...
	/**
	 * This method will use the internal PD velocity controller to run a PID
	 * controller at a constant velocity. Since this is not using the linear
	 * interpolation, it can run forever by giving Zero as the 'seconds' parameter.
	 *
	 * @param group
	 *            the index of the PID group
	 * @param unitsPerSecond
	 *            a velocity in raw units per second
	 * @param seconds
	 *            the amount of time to run at this velocity, or Zero to run forever
	 * @return true, if successful
	 * @throws PIDCommandException
	 *             If the values are out of range with the given data
	 */
	 ...
	 * will encapsulate a specific PID channel.
	 *
	 * @param group
	 *            the index of the PID group
