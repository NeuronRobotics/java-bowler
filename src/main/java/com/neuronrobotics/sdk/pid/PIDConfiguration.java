...
public class PIDConfiguration {
	 ...
	 *
	 * @param group
	 *            This is the PID group this configuration object represents
	 * @param enabled
	 *            True if the controller is running, false otherwise
	 * @param inverted
	 *            This inverts the output value. Set true if the controller diverges
	 * @param async
	 *            sets the flag to send this channels async values upstream
	 * @param KP
	 ...
	 *            Derivative constant
	 * @param latch
	 *            The value to latch into the PID controller if the home switch is
	 *            hit (encoder only, not used in analog PID)
	 * @param useLatch
	 *            Use the value to latch into the PID controller if the home switch
	 *            is hit (encoder only, not used in analog PID)
	 * @param stopOnLatch
	 *            Set the setpoint of the controller to current if home switch is
	 *            hit (encoder only, not used in analog PID)
	 * @param up
	 *            the up
	 * @param low
	 ...
	 *            the h stop
	 */
	public PIDConfiguration(int group, boolean enabled, boolean inverted, boolean async, double KP, double KI,
			double KD, double latch, boolean useLatch, boolean stopOnLatch, double up, double low, double hStop) {
		setGroup(group);
		setEnabled(enabled);
	 ...
	 */
	public PIDConfiguration(int group, boolean enabled, boolean inverted, boolean async, double KP, double KI,
			double KD, double latch, boolean useLatch, boolean stopOnLatch) {
	...
	public Object[] getArgs() {
		Object[] args = new Object[]{group, enabled ? 1 : 0, inverted ? 1 : 0, async ? 1 : 0, KP, KI, KD, latch,
				useLatch ? 1 : 0, stopOnIndex ? 1 : 0, hStop, up, low};
		return args;
	}
