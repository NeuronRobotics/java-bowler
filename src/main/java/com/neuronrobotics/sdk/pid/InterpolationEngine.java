...
public class InterpolationEngine extends TimeKeeper {
	...
	public void setSetpointWithTime(long startTimeMs, double setpoint, double seconds, InterpolationType mode,
			double... conf) {
	 ...
	 *
	 * @param newTargetInDegrees
	 *            the new setpoint for the closed loop controller
	 * @param miliseconds
	 *            the number of miliseconds to get from current position to the new
	 *            setpoint
	 * @param Control_0
	 *            On a scale of 0 to 1, where should the first control point in the
	 *            equation go default= 0.5
	 * @param Control_1
	 *            On a scale of 0 to 1, where should the second control point in the
	 *            equation go default= 1.0 use Bezier interpolation
	 */
		...
		if (interpElapsed < duration && duration > 0) {
			...
			if (type == InterpolationType.BEZIER) {
				if (getUnitDuration() > 0 && getUnitDuration() < 1) {
					...
					p2 = BEZIER_P1;
					p3 = 1;
					setUnitDuration(Math.pow((1 - t), 3) * p0 + 3 * t * Math.pow((1 - t), 2) * p1
							+ 3 * Math.pow(t, 2) * (1 - t) * p2 + Math.pow(t, 3) * p3);
				}
			}
	...
	public void setTicks(double ticks) {
		...
		// new RuntimeException("Ticks in virtual device can not be set to
		// nan").printStackTrace();
		// return;
		// }
		this.ticks = ticks;
