...
public class BlueCoveManager implements DiscoveryListener {
	 ...
	 * Find.
	 *
	 * @throws MissingNativeLibraryException
	 *             the missing native library exception
	 */
	...
	public void deviceDiscovered(RemoteDevice btDevice, DeviceClass cod) {
		...
		// Log.info("deviceDiscovered " + btDevice.getBluetoothAddress() + "
		// DeviceClass: " + ((Object)cod).toString());
	}
	...
	public synchronized RemoteDevice getDevice(String name) {
		...
		// com.neuronrobotics.sdk.common.Log.error("Getting device with address:
		// "+addr);
	...
	private synchronized boolean connect() {
		...
		synchronized (this) {
			...
			try {
				...
				if (searchId != 0xffff)
					LocalDevice.getLocalDevice().getDiscoveryAgent().cancelServiceSearch(searchId);
				searchId = LocalDevice.getLocalDevice().getDiscoveryAgent().searchServices(attrSet, id,
						getDevice(selected), this);
				try {
					wait();
				} catch (InterruptedException e) {
					e.printStackTrace();
