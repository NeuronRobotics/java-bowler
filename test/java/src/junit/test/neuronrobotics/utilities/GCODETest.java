...
public class GCODETest {
	...
	// if (hasPort) {
	// GcodeDevice device =
	// GCODECONTOLLER.cast(DeviceManager.getSpecificDevice(GCODECONTOLLER, GCODE));
	// device.disconnect();
	// }
	// }
	//
	...
	//
	// MobileBase cnc = new
	// MobileBase(GCODETest.class.getResourceAsStream("cnc.xml"));
	// DHParameterKinematics arm = cnc.getAppendages().get(0);
	// arm.setInverseSolver(new DhInverseSolver() {
	// @Override
	// public double[] inverseKinematics(TransformNR target, double[]
	// jointSpaceVector, DHChain chain) {
	...
	// try {
	// arm.setDesiredTaskSpaceTransform(new TransformNR(10, 10, 0, new
	// RotationNR()), 1);
